...
// Bins to study
string xquantity = "Pt";
double xbins[] = {15.0,20.0,25.0,30.0,40.0,50.0,60.0};
...
void plot_sys_efficiency_2d()
{
	...
	TH2D *hist_all_2voitgtian     = create_TH2D("all_2xVoitgtian"   ,  "All 2xVoitgtian",     xquantity, yquantity, nbinsx, nbinsy, xbins, ybins);
	...
	TH2D *hist_pass_2voitgtian    = create_TH2D("pass_2xVoitgtian"   , "Pass 2xVoitgtian",    xquantity, yquantity, nbinsx, nbinsy, xbins, ybins);
	...
	for (int i = 0; i < nbinsx; i++)
	{
		for (int j = 0; j < nbinsy; j++)
		{
			...
			//2voit
			cout << "2xVoitgtian calculation -----\n";
			_mmin = default_min;
			_mmax = default_max;
			fit_bins = 100;
			prefix_file_name = "2xVoitgtian_";
			yields_n_errs = doFit2xVoitgtian(conditions, MuonId, string(path_bins_fit_folder + prefix_file_name).c_str());
			yields_n_errs_systematic[2] += pow(yields_n_errs[2], 2);
			yields_n_errs_systematic[3] += pow(yields_n_errs[3], 2);
			yields_n_errs_to_TH2Ds_bin(hist_all_2voitgtian, hist_pass_2voitgtian , i+1, j+1, yields_n_errs);

			//MassUp
			cout << "MassUp calculation -----\n";
			_mmin = default_min - 0.06;
			_mmax = default_max + 0.06;
			fit_bins = 100;
			...
			prefix_file_name  = string("mass_") + mmin_string.substr(0, mmin_string.length()-10) + string("_");
			prefix_file_name +=                   mmax_string.substr(0, mmax_string.length()-10) + string("_");
			yields_n_errs = doFit(conditions, MuonId, string(path_bins_fit_folder + prefix_file_name).c_str());
			yields_n_errs_systematic[2] += pow(yields_n_errs[2], 2);
			yields_n_errs_systematic[3] += pow(yields_n_errs[3], 2);
	...
	get_efficiency_TH2D(hist_all_2voitgtian,     hist_pass_2voitgtian,     xquantity, yquantity, MuonId, "2xVoitgtian"   );
	...
	generatedFile->cd("/");
	get_efficiency_TH2DJpsi(hist_all_nominal,    hist_pass_nominal,    xquantity, yquantity, MuonId, "Nominal"   );
	get_efficiency_TH2DJpsi(hist_all_2gauss,     hist_pass_2gauss,     xquantity, yquantity, MuonId, "2xGauss"   );
	get_efficiency_TH2DJpsi(hist_all_massup,     hist_pass_massup,     xquantity, yquantity, MuonId, "MassUp"    );
	get_efficiency_TH2DJpsi(hist_all_massdown,   hist_pass_massdown,   xquantity, yquantity, MuonId, "MassDown"  );
	get_efficiency_TH2DJpsi(hist_all_binup,      hist_pass_binup,      xquantity, yquantity, MuonId, "BinUp"     );
	get_efficiency_TH2DJpsi(hist_all_bindown,    hist_pass_bindown,    xquantity, yquantity, MuonId, "BinDown"   );
	get_efficiency_TH2DJpsi(hist_all_systematic, hist_pass_systematic, xquantity, yquantity, MuonId, "Systematic");
	get_efficiency_TH2DJpsi(hist_all_final,      hist_pass_final,      xquantity, yquantity, MuonId, "Final"     );
