...
//string quantity = "Eta";    double bins[] = {-0.8, -0.5};
...
void plot_sys_efficiency_Z()
{
	...
	double** yields_n_errs_2xVoitgtian   = new double*[nbins];
	...
	for (int i = 0; i < nbins; i++)
	{
		...
		yields_n_errs_2xVoitgtian[i] = doFit2xVoitgtian(conditions, MuonId, (path_bins_fit_folder + prefix_file_name).c_str());
		...
		prefix_file_name  = string("massUp_") + mmin_string.substr(0, mmin_string.length()-4) + string("_");
		...
		replace(mmin_string.begin(), mmin_string.end(), '.', 'p');
		replace(mmax_string.begin(), mmax_string.end(), '.', 'p');
		prefix_file_name  = string("massDown_") + mmin_string.substr(0, mmin_string.length()-4) + string("_");
		prefix_file_name +=                   mmax_string.substr(0, mmax_string.length()-4) + string("_");
		...
		result[1] = yields_n_errs_2xVoitgtian[i][1];
		result[2] = sqrt(pow(yields_n_errs_Nominal[i][2],2) + pow(yields_n_errs_2xVoitgtian[i][2],2) + pow(yields_n_errs_MassUp[i][2],2) + pow(yields_n_errs_MassUp[i][2],2) + pow(yields_n_errs_BinUp[i][2],2) + pow(yields_n_errs_BinDown[i][2],2));
		result[3] = sqrt(pow(yields_n_errs_Nominal[i][3],2) + pow(yields_n_errs_2xVoitgtian[i][3],2) + pow(yields_n_errs_MassUp[i][3],2) + pow(yields_n_errs_MassUp[i][3],2) + pow(yields_n_errs_BinUp[i][3],2) + pow(yields_n_errs_BinDown[i][3],2));
	...
	TH1D* hist_2xvoitgtian_all    = make_TH1D("all_2xVoitgtian"  , yields_n_errs_2xVoitgtian , 0, bins, nbins, quantity);
	...
	TH1D* hist_nominal_pass  = make_TH1D("pass_nominal" , yields_n_errs_Nominal  , 1, bins, nbins, quantity);
	TH1D* hist_2xvoitgtian_pass    = make_TH1D("pass_2xVoitgtian" , yields_n_errs_2xVoitgtian , 1, bins, nbins, quantity);
	...
	TH1D* hist_binup_pass    = make_TH1D("pass_BinUp" , yields_n_errs_BinUp   , 1, bins, nbins, quantity);
	...
	get_efficiency(hist_nominal_all , hist_nominal_pass   , quantity, MuonId, "Nominal" , true);
	get_efficiency(hist_2xvoitgtian_all   , hist_2xvoitgtian_pass , quantity, MuonId, "2xVoitgtian" , true);
